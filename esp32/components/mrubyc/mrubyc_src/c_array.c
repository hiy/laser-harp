...
static void c_array_index(struct VM *vm, mrbc_value v[], int argc)
{
  ...
  for (i = 0; i < n; i++)
  {
    if (mrbc_compare(&data[i], value) == 0)
      break;
  }
...
static void c_array_join(struct VM *vm, mrbc_value v[], int argc)
{
  ...
  mrbc_value separator = (argc == 0) ? mrbc_string_new_cstr(vm, "") : mrbc_send(vm, v, argc, &v[1], "to_s", 0);
